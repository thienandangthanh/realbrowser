...
- Anonymous mode uses Chrome DevTools MCP isolated state by default. Managed anonymous/dedicated sessions are headless unless `--headed`, `--front`, or `REALBROWSER_HEADLESS=0` is used, and idle-shutdown after `REALBROWSER_IDLE_TIMEOUT_MS` milliseconds (default 30 minutes, `0` disables). `--keep-anonymous` uses a temporary `userDataDir` for inspection after detach. Neither mode masks IP address, device/browser fingerprint, or network identity.
